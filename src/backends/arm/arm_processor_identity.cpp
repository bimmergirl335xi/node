...
ArmProcessorIdentityQueryResult parse_arm_proc_cpuinfo(
    ...
    const std::size_t complete_signature_count =
        static_cast<std::size_t>(std::count_if(
            snapshot.identities.begin(),
            snapshot.identities.end(),
            [](const ArmProcessorIdentity& identity) {
                return identity.signature_complete();
            }));
    snapshot.heterogeneous = complete_signature_count > 1;
