...
over-limit input, and returns a declaration-state/failure result. It has no
launch or runtime lifecycle state.
...
Supplying both is rejected. Supplying neither succeeds with
`declaration_not_supplied`, which is the normal public-only result. The
provider performs no directory
