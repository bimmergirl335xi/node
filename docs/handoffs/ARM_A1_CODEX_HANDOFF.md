...
### 3. Stitch processor identity into the aggregate snapshot

The isolated branch intentionally leaves processor-identity discovery separate from `query_arm_capabilities()`.
...
Then integrate conservatively:

```cpp
snapshot.processor_identities = identity_result.snapshot.identities;
```

Map identity issues into the aggregate issue model without discarding their original code, CPU ID, fatality, or message.

After successful identity integration, remove the unconditional placeholder issue currently emitted by `query_arm_capabilities()`:

```text
identity_unavailable: ARM processor identity discovery is not implemented in this pass
```

Do not convert a partial identity result into a fully successful query. Preserve partial status when target records are missing or incomplete.
...
```cmake
add_executable(arm_capability_probe
    arm_capability_probe.cpp
)

target_link_libraries(arm_capability_probe
    PRIVATE
        prometheus_arm_capabilities
        prometheus_cpu_foundation
)

target_compile_features(arm_capability_probe PRIVATE cxx_std_17)
```
...
- [ ] ARM CPU is represented once through the generic CPU backend.
- [ ] Auxiliary-vector observations compile on non-ARM Linux.
- [ ] Missing HWCAP/HWCAP2 remains unknown rather than unsupported.
- [ ] Identity parser retains only target logical processors.
- [ ] Multiple complete signatures produce multiple groups.
- [ ] Processor signatures do not select instruction implementations.
- [ ] Aggregate status preserves partial discovery.
- [ ] All three ARM tests pass in the integrated tree.
- [ ] Probe builds and runs on x86-64.
- [ ] Probe runs on real AArch64 hardware.
- [ ] No Hailo, IMX500, camera, Raspberry Pi, or GPU assumption is introduced.
- [ ] No worker-pool, scheduling, backend-registry, or robot behavior regression is introduced.
